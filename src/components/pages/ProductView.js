...
class ProductView extends Component {
    constructor(props) {
        super(props);
        this.state = {
            activeId: window.location.href.split("/")[window.location.href.split("/").length-1]
        }
    }

    // componentDidMount () {
    //     console.log(window.location.href.split("/"));
    //     let activeIDTest = window.location.href.split("/");
    //     this.setState({activeId: activeIDTest[activeIDTest.length - 1]})
    // }
                                ...
                                <li class="breadcrumb-item active">
                                    ...
                                    {productData.map(item=> {
                                        if(item.id == this.state.activeId) {
                                            return (
                                                <a href="https://olcha.uz/oz/category/telefony-gadzhety-aksessuary/smart-watches">{item.nameCategory}</a>
                                            )
                                        }
                                    })}
                                </li>
                                        ...
                                        <main className="col-md-6">
                                            <article className='product-info-aside'>
                                                ...
                                                <div className="availabled">
                                                    <div className="mb-4">
                                                        <del className="price-old monthly">399 000 so‘m</del>
                                                        <br />
                                                        <var className="price h4">339 000 so‘m</var>
                                                        <br />
                                                        <span className="monthly">
                                                            <span>
                                                                41000
                                                            </span>
                                                            <span id="installment_sum" hidden="">97 000 </span>
                                                            so‘m/oy 
                                                            <a 
                                                                href="#" 
